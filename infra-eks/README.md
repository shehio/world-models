...
1. **Spot reclamation resilience.** The managed node group spans
   ...
   completion index automatically (up to `backoffLimit: 24`). No
   bespoke retry loops.
...
### Self-play, MuZero, and Go

| File | Role |
|---|---|
| `k8s/job-selfplay.yaml` / `entrypoint-selfplay.sh` / `run-selfplay.sh` | Chess self-play RL from a distilled prior: Job manifest, container entrypoint, orchestrator |
| `cluster-selfplay-us.yaml` | eksctl spec for the self-play GPU cluster (separate lifecycle from the supervised-training clusters) |
| `entrypoint-muzero-chess.sh` / `entrypoint-muzero-distill-init.sh` | MuZero entrypoints (from-scratch / dynamics-only on the frozen teacher), fired by the `launchers/muzero-*.sh` launchers |
| `Dockerfile.wm-go` / `Dockerfile.wm-go-gpu` | Go images (KataGo CPU / GPU + project), built via `buildspec-go.yml` / `buildspec-go-gpu.yml` |
| `entrypoint-go.sh` | Go datagen / merge mode dispatcher inside the wm-go containers |
| `k8s/job-gen-go.yaml` / `k8s/job-gen-go-gpu.yaml` | Go 9×9 datagen Jobs (CPU spot / one-GPU-per-pod) |
| `k8s/service-account-go.yaml` | SA for the Go Jobs |
| `cluster-gen-go-use2.yaml` / `cluster-gen-go-gpu-use1.yaml` / `cluster-gen-go-gpu-use2.yaml` | eksctl specs for the Go datagen clusters |
| `katago-analysis.cfg` / `katago-analysis-gpu.cfg` | KataGo analysis-engine configs baked into the Go images |
| `spot-rover/` | Region scanner: scores spot capacity across regions and deploys the datagen job to the winner ([own README](./spot-rover/README.md)) |
| `cleanup-zombie-clusters.sh` | Scans regions for idle EKS clusters and deletes them (dry-run by default) |
