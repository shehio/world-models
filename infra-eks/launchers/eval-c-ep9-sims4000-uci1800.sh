#!/usr/bin/env bash
# Epoch-vs-search interaction for C ep9: 4000 sims @ UCI=1800.
#
# At sims=800, ep9 scored 2,009 (vs ep4's 2,004) at UCI=1800 — basically
# the same, with ep9 slightly ahead. Does deeper search amplify or
...
docker run --rm \\
    ...
    \$ECR_IMAGE -lc '
        ...
        OUT=/work-tmp/eval_results-ep9-sims4000-uci1800.txt
        echo "=== C ep9 vs Stockfish UCI=1800 (sims=4000) ===" | tee \$OUT
...
aws ec2 run-instances --region $LAUNCH_REGION \
    ...
    --user-data "$USER_DATA" \
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=wm-eval-c-ep9-sims4000-uci1800},{Key=role,Value=wm-chess-eval}]" \
